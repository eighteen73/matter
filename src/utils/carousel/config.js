import ClassNames from 'embla-carousel-class-names';
import Autoplay from 'embla-carousel-autoplay';
import AutoScroll from 'embla-carousel-auto-scroll';
import Fade from 'embla-carousel-fade';

import breakpoints, {
	breakpointTokens,
	minWidthQuery,
} from '../../constants/breakpoints/data';

export const DEFAULT_CAROUSEL_CONFIG = {
	options: {
		loop: false,
		axis: 'x',
		slidesToScroll: 1,
		slidesToShow: 1,
		slideGap: '',
		active: true,
	},
	plugins: {
		fade: {
			active: false,
		},
		autoplay: {
			active: false,
			type: 'slide',
			delay: 4000,
		},
	},
	breakpointLayers: {},
};

/** @deprecated Use DEFAULT_CAROUSEL_CONFIG. */
export const DEFAULT_EMBLA_CONFIG = DEFAULT_CAROUSEL_CONFIG;

const isPlainObject = (value) =>
	value !== null && typeof value === 'object' && !Array.isArray(value);

const deepMerge = (target, source) => {
	if (!isPlainObject(source)) {
		return isPlainObject(target) ? { ...target } : {};
	}
	const base = isPlainObject(target) ? { ...target } : {};
	for (const key of Object.keys(source)) {
		const next = source[key];
		const prev = base[key];
		base[key] =
			isPlainObject(prev) && isPlainObject(next)
				? deepMerge(prev, next)
				: next;
	}
	return base;
};

/** @param {Object|undefined} input */
export const normalizeCarouselConfig = (input) =>
	deepMerge(DEFAULT_CAROUSEL_CONFIG, isPlainObject(input) ? input : {});

/** @deprecated Use normalizeCarouselConfig. */
export const normalizeEmblaConfig = normalizeCarouselConfig;

/**
 * Compile UI `breakpointLayers` into carousel `options.breakpoints`,
 * `plugins.autoplay.breakpoints`, and `plugins.fade.breakpoints`
 * (ascending min-width).
 * @param {Object} resolved - The resolved carousel config.
 * @param {Object} layers   - The breakpoint layers.
 */
const applyBreakpointLayers = (resolved, layers) => {
	const safeResolved = isPlainObject(resolved) ? resolved : {};
	const safeLayers = isPlainObject(layers) ? layers : {};
	const options = isPlainObject(safeResolved.options)
		? { ...safeResolved.options }
		: {};
	const plugins = isPlainObject(safeResolved.plugins)
		? { ...safeResolved.plugins }
		: {};

	const optionBreakpoints = isPlainObject(options.breakpoints)
		? { ...options.breakpoints }
		: {};

	let autoplay = isPlainObject(plugins.autoplay)
		? { ...plugins.autoplay }
		: {};
	const autoplayBreakpoints = isPlainObject(autoplay.breakpoints)
		? { ...autoplay.breakpoints }
		: {};

	let fade = isPlainObject(plugins.fade) ? { ...plugins.fade } : {};
	const fadeBreakpoints = isPlainObject(fade.breakpoints)
		? { ...fade.breakpoints }
		: {};

	for (const token of breakpointTokens) {
		const layer = safeLayers[token];
		const minWidth = breakpoints[token]?.value;

		if (!isPlainObject(layer) || !minWidth) {
			continue;
		}

		const query = minWidthQuery(minWidth);

		if (isPlainObject(layer.options) && Object.keys(layer.options).length) {
			optionBreakpoints[query] = isPlainObject(optionBreakpoints[query])
				? deepMerge(optionBreakpoints[query], layer.options)
				: { ...layer.options };
		}

		const autoplayLayer = layer.plugins?.autoplay;
		if (isPlainObject(autoplayLayer) && Object.keys(autoplayLayer).length) {
			autoplayBreakpoints[query] = isPlainObject(
				autoplayBreakpoints[query]
			)
				? deepMerge(autoplayBreakpoints[query], autoplayLayer)
				: { ...autoplayLayer };
		}

		const fadeLayer = layer.plugins?.fade;
		if (isPlainObject(fadeLayer) && Object.keys(fadeLayer).length) {
			fadeBreakpoints[query] = isPlainObject(fadeBreakpoints[query])
				? deepMerge(fadeBreakpoints[query], fadeLayer)
				: { ...fadeLayer };
		}
	}

	if (Object.keys(optionBreakpoints).length) {
		options.breakpoints = optionBreakpoints;
	}
	if (Object.keys(autoplayBreakpoints).length) {
		autoplay = { ...autoplay, breakpoints: autoplayBreakpoints };
		plugins.autoplay = autoplay;
	}
	if (Object.keys(fadeBreakpoints).length) {
		fade = { ...fade, breakpoints: fadeBreakpoints };
		plugins.fade = fade;
	}

	return { options, plugins };
};

/**
 * @param {Object}  args
 * @param {Object}  [args.carouselConfig]
 * @param {Object}  [args.advancedCarouselConfig]
 * @param {boolean} [args.advancedCarouselConfigMerge]
 * @param {Object}  [args.emblaConfig]                 - Alias for carouselConfig (carousel block).
 * @param {Object}  [args.advancedEmblaConfig]         - Alias for advancedCarouselConfig.
 * @param {boolean} [args.advancedEmblaConfigMerge]    - Alias for advancedCarouselConfigMerge.
 */
export const prepareCarouselBlockState = ({
	carouselConfig,
	advancedCarouselConfig,
	advancedCarouselConfigMerge,
	emblaConfig,
	advancedEmblaConfig,
	advancedEmblaConfigMerge,
} = {}) => {
	const config = carouselConfig ?? emblaConfig;
	const advancedConfig = advancedCarouselConfig ?? advancedEmblaConfig;
	const mergeAdvanced =
		advancedCarouselConfigMerge ?? advancedEmblaConfigMerge;

	const base = normalizeCarouselConfig(config);
	const advanced = isPlainObject(advancedConfig) ? advancedConfig : {};
	const mergeWithBase = mergeAdvanced === true;
	const hasAdvanced = Object.keys(advanced).length > 0;

	let resolved;
	if (mergeWithBase) {
		resolved = deepMerge(base, advanced);
	} else if (hasAdvanced) {
		resolved = deepMerge(DEFAULT_CAROUSEL_CONFIG, advanced);
	} else {
		resolved = base;
	}

	const compiled = applyBreakpointLayers(resolved, resolved.breakpointLayers);

	return {
		carouselOptions: compiled.options,
		emblaOptions: compiled.options,
		pluginState: compiled.plugins,
	};
};

/** @deprecated Use prepareCarouselBlockState. */
export const prepareEmblaBlockState = prepareCarouselBlockState;

/**
 * Register ClassNames, optional Fade, plus Autoplay and AutoScroll
 * (mutually exclusive via `active` / per-breakpoint `type`) so responsive
 * autoplay type can switch.
 * @param {Object}  pluginState             - The plugin state.
 * @param {Object}  [options]               - Build options.
 * @param {boolean} [options.forceInactive] - Disable Autoplay/AutoScroll only
 *                                          (e.g. in the editor). Fade is
 *                                          unaffected so transition preview works.
 */
export const buildCarouselPlugins = (pluginState, options = {}) => {
	const { forceInactive = false } = options;
	const plugins = [ClassNames()];
	const fade = isPlainObject(pluginState?.fade) ? pluginState.fade : {};
	const autoplay = isPlainObject(pluginState?.autoplay)
		? pluginState.autoplay
		: {};
	const {
		active: fadeBaseActive = false,
		breakpoints: fadeRawBreakpoints,
		...fadeRest
	} = fade;
	const {
		active: baseActive = false,
		type: baseType = 'slide',
		breakpoints: rawBreakpoints,
		...rest
	} = autoplay;

	const wantBaseActive = !forceInactive && !!baseActive;
	const autoplayBaseActive = wantBaseActive && baseType !== 'scroll';
	const autoScrollBaseActive = wantBaseActive && baseType === 'scroll';

	const fadeBreakpoints = {};

	if (isPlainObject(fadeRawBreakpoints)) {
		for (const [query, rawLayer] of Object.entries(fadeRawBreakpoints)) {
			if (!isPlainObject(rawLayer)) {
				continue;
			}
			const { active: layerActiveRaw, ...layerRest } = rawLayer;
			const effectiveActive =
				layerActiveRaw !== undefined
					? !!layerActiveRaw
					: !!fadeBaseActive;
			fadeBreakpoints[query] = {
				...layerRest,
				active: effectiveActive,
			};
		}
	}

	const fadeOptions = {
		...fadeRest,
		active: !!fadeBaseActive,
	};
	if (Object.keys(fadeBreakpoints).length) {
		fadeOptions.breakpoints = fadeBreakpoints;
	}
	plugins.push(Fade(fadeOptions));

	const autoplayBreakpoints = {};
	const autoScrollBreakpoints = {};

	if (isPlainObject(rawBreakpoints)) {
		for (const [query, rawLayer] of Object.entries(rawBreakpoints)) {
			if (!isPlainObject(rawLayer)) {
				continue;
			}
			const {
				active: layerActiveRaw,
				type: layerTypeRaw,
				...layerRest
			} = rawLayer;
			const effectiveActive =
				layerActiveRaw !== undefined ? !!layerActiveRaw : !!baseActive;
			const effectiveType =
				layerTypeRaw !== undefined ? layerTypeRaw : baseType;
			const wantLayerActive = !forceInactive && effectiveActive;
			autoplayBreakpoints[query] = {
				...layerRest,
				active: wantLayerActive && effectiveType !== 'scroll',
			};
			autoScrollBreakpoints[query] = {
				...layerRest,
				active: wantLayerActive && effectiveType === 'scroll',
			};
		}
	}

	const sharedOptions = { speed: 1, ...rest };

	const autoplayOptions = {
		...sharedOptions,
		active: autoplayBaseActive,
	};
	if (Object.keys(autoplayBreakpoints).length) {
		autoplayOptions.breakpoints = autoplayBreakpoints;
	}
	plugins.push(Autoplay(autoplayOptions));

	const autoScrollOptions = {
		...sharedOptions,
		active: autoScrollBaseActive,
	};
	if (Object.keys(autoScrollBreakpoints).length) {
		autoScrollOptions.breakpoints = autoScrollBreakpoints;
	}
	plugins.push(AutoScroll(autoScrollOptions));

	return plugins;
};

/** @deprecated Use buildCarouselPlugins. */
export const buildEmblaPlugins = buildCarouselPlugins;
